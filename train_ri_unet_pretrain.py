...
class ColdRIUNetTrainer:
    ...
    def __init__(self, model, pre_params, train_params, ad2_data, output_dir, pretrained_ckpt=None):
        """Initializer.
        Args:
            model: UNet RI
        ...
        # Load pretrained checkpoints or restore from previous training
        if self.pretrained_ckpt:
            try:
                ckpt.restore(self.pretrained_ckpt).expect_partial()
                print(f"Pretrained checkpoint restored from {self.pretrained_ckpt}")
            except Exception as e:
                print(f"Failed to load pretrained checkpoint: {e}")
