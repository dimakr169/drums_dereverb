...
def main():
    ...
    def _wav_from_ri(trainer, ri):  # (B,4,F,T)->(B,2,T)
        return trainer.get_signal_from_RI_stft(ri)

    @torch.no_grad()
    def reverse_diffusion_full(trainer, model, x_init, step_stop=0, scale_override=None):
        """
        x_init: (1,4,F,T) starting at t=T (reverberant/noised)
        Returns list of RI tensors [x_T, x_{T-1}, ..., x_{step_stop}]
        """
        model.eval()
        xs = []
        x = x_init.clone()
        T = trainer.diffusion_steps
        for t in range(T, step_stop, -1):
            B = x.size(0)
            tt = torch.full((B,), t, device=x.device, dtype=torch.long)
            a_t   = trainer.alpha_bar.index_select(0, tt)
            a_tm1 = trainer.alpha_bar.index_select(0, tt-1)
            g = (a_tm1 - a_t).clamp_min(1e-6).view(-1,1,1,1)
            out = model(x, tt)

            if trainer.residual_mode:
                x = x +  g * out   
            else:
                x = out                           # direct x_{t-1}
            xs.append(x.detach().clone())
        return xs  # [x_{T-1}, x_{T-2}, ..., x_{0}]
    

    # ---------------------------------------------------------
    # Build optimizer + EMA for the overfit test
    opt = torch.optim.AdamW(filter(lambda p: p.requires_grad, trainer.model.parameters()), lr=3e-4)
    ema = EMAModel(trainer.model, decay=0.9)  # start low; this is key to test EMA impact early
    ...
    # Overfit steps
    out_root = Path("./overfit_reverse_test")
    (out_root / "noema").mkdir(parents=True, exist_ok=True)
    (out_root / "ema").mkdir(parents=True, exist_ok=True)

    def _save_triplet(dirpath, it, x_t, pred, x_tm1, sr=44100):
        dirpath = Path(dirpath) / f"{it:04d}"
        dirpath.mkdir(parents=True, exist_ok=True)
        def save_wav(path, wav_2ch):
            sf.write(str(path), wav_2ch[0].detach().squeeze(0).permute(1,0).cpu().numpy(), sr)
        save_wav(dirpath / "input.wav",  _wav_from_ri(trainer, x_t))
        save_wav(dirpath / "pred.wav",   _wav_from_ri(trainer, pred))
        save_wav(dirpath / "target.wav", _wav_from_ri(trainer, x_tm1))

    print("Starting 1-batch overfit with EMA tracking...")
    steps = 3000
    for step in range(steps):
        # Pick a SINGLE random t per sample (same as your test)
        timesteps = trainer._random_timesteps(B)  # (B,)
        a_t, a_tm1 = trainer._levels_for(timesteps)
        ...
        if trainer.residual_mode:
            g = (a_tm1 - a_t).clamp_min(1e-6).view(-1,1,1,1)       # (B,1,1,1)
            est_v   = trainer.model(x_t , timesteps)                # v̂_t
            pred  = x_t  + g * est_v                           # x̂_{t-1}
            target_v = (x_tm1 - x_t ) / g
            loss_ri = F.l1_loss(est_v, target_v) * 30.0 # 
        else:
            pred = trainer.model(x_t, timesteps) 
            loss_ri = F.l1_loss(pred, x_tm1) * 30.0

        loss_w  = F.l1_loss(_wav_from_ri(trainer, pred), _wav_from_ri(trainer, x_tm1)) * 100.0
        loss = loss_ri + loss_w
        loss.backward()
        opt.step()
        ema.update()  # <<< track EMA each step

        if step % 200 == 0:
            print(f"[overfit] step {step:04d} | RI {loss_ri.item():.4f} | Aud {loss_w.item():.4f}")
            _save_triplet(out_root/"noema", step, x_t, pred, x_tm1)

    # ---------------------------------------------------------
    # Now do FULL reverse diffusion on SAMPLE 0 with and without EMA

    with torch.no_grad():
        # Build the starting point x_T for that sample from your chosen t=T schedule input
        # For “true” reverse test, use the actual reverberant input (at t=T):
        x_T = reverb_ri[0:1]  # (1,4,F,T)

    # (A) WITHOUT EMA
    xs_noema = reverse_diffusion_full(trainer, trainer.model, x_T)  # list of RI steps
    # Save per-step WAVs
    for k, ri in enumerate(xs_noema):
        wav = _wav_from_ri(trainer, ri)
        sf.write(str(out_root/"noema"/f"step_{k:02d}.wav"),
                wav[0].detach().squeeze(0).permute(1,0).cpu().numpy(), 44100)

    # (B) WITH EMA (swap in, run reverse, then restore)
    ema.apply_shadow()
    xs_ema = reverse_diffusion_full(trainer, trainer.model, x_T)
    ema.restore()
    for k, ri in enumerate(xs_ema):
        wav = _wav_from_ri(trainer, ri)
        sf.write(str(out_root/"ema"/f"step_{k:02d}.wav"),
                wav[0].detach().squeeze(0).permute(1,0).cpu().numpy(), 44100)

    print("Saved reverse sequences to:", out_root.resolve())
