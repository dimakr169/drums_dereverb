...
# ---- alpha schedule: cos^2 ----
def make_alpha_bar(diffusion_steps: int, device):
    t = torch.arange(diffusion_steps+1, device=device, dtype=torch.float32)
    return torch.cos(0.5 * math.pi * t / diffusion_steps).pow(2)
...
def build_scheduler(optimizer, policy: str, base_lr: float, steps_per_epoch: int,
    ...
    if policy == "cosine_restart":
        # PyTorch uses CosineAnnealingWarmRestarts in *iterations* (steps)
    ...
    if policy == "warmup_cosine":
        ...
        def lr_lambda(step):
            if step < warmup_steps:
                ...
                return max(warmup_initial_lr, 1e-12) / base_lr + (step / max(1, warmup_steps)) * (1.0 - max(warmup_initial_lr, 1e-12)/base_lr)
            # cosine decay to floor factor
            k = (step - warmup_steps) / cosine_len
            return cosine_floor_factor + 0.5 * (1 - cosine_floor_factor) * (1 + math.cos(math.pi * k))
...
class ColdRIUNetTrainer: 
    def __init__(self, model, pre_params, train_params, dataloaders, output_dir, device="cuda"):
        ...
        # alpha_bar[0..T]
        self.alpha_bar = make_alpha_bar(self.diffusion_steps, device=self.device)
    ...
    def diffusion(self, reverb_ri, clean_ri, noise_level):
        # reverb_ri/clean_ri: (B,4,F,T), noise_level: (B,) in [0..1]
    ...
    def _step(self, batch, train=True, global_step=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.device.startswith("cuda")):
            est_ri = self.model(noised, timesteps)  # (B,4,F,T)
            # Noise-domain MAE for all 4 channels
        ...
        for epoch in range(self.train_params.epochs):
            ...
            with torch.no_grad():
                for batch in self.val_loader:
                    ...
                    n_batches += 1
                    # SI metrics (stubs)
                    self.sisdr.update(out["tar_wav"], out["est_wav"])
                    self.sisdri.update(out["tar_wav"], out["est_wav"], out["inp_wav"]) 
...
def main():
    ...
    parser.add_argument("--model-name", default="CDiff_RI_s_1024-384_32ch")
    ...
    trainer = ColdRIUNetTrainer(model, pre_params, train_params, dataloaders, out_dir, device=device)
    trainer.train()
